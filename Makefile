...
vastai_run:
	@ssh -o StrictHostKeyChecking=accept-new `cat ssh_url.txt` "apt install -y python3-pip nano vim git; pip3 install numpy cupy-cuda12x matplotlib scipy dask distributed moviepy pycuda; rm -rf electronwaves; git clone https://github.com/vincecate/electronwaves; cd electronwaves && mkdir -p results && chmod +x go.save && chmod +x makemovie.wire.nc.py && python3 electronwaves.py $(SIMULATION_NUMBER) && bash go.save $(SIMULATION_NUMBER);"
